...
#         )
#         total_mrs = sum(
#             count_items_by_date(pdata.get("mrs", []), "created_at", start_date, end_date)
#             for pdata in contrib_data.values()
#         )
#         total_issues = sum(
#             count_items_by_date(pdata.get("issues", []), "created_at", start_date, end_date)
#             for pdata in contrib_data.values()
#         )
#         total_mr_comments = sum(
#             sum(m.get("comment_count", 0) for m in pdata.get("mrs", []))
#             for pdata in contrib_data.values()
#         )
#         total_issue_comments = sum(
#             sum(i.get("comment_count", 0) for i in pdata.get("issues", []))
#             for pdata in contrib_data.values()
#         )
...
#                         all_data = process_csv_data_cached(
#                             current_hash, 
#                             usernames, 
...
#                         status_text.text("✅ Data loaded from cache!")
#                     except Exception as e:
#                         # Fallback to individual fetching with progress
#                         all_data = []
...
#             ["All Metrics", "Commits", "Merge Requests", "Total Contributions"],
#             key="metric_filter"
#         )
...
#     with col4:
#         st.metric("Avg Total Contributions", f"{df_filtered['total_contributions'].mean():.1f}")
...
async def fetch_issues(session, user_id, project_id):
    return await fetch_json(
        session,
        f"{BASE_URL}/projects/{project_id}/issues",
        params={"author_id": user_id, "state": "all"},  # Added state=all to get all issues
    )
